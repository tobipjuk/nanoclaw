#!/usr/bin/env bash
# Obsidian vault disaster recovery backup
# Encrypts vault snapshot and uploads to OneDrive. Retains 7 daily backups.
set -euo pipefail

LOG_FILE="/root/nanoclaw/logs/backup.log"
TIMESTAMP=$(date -u +%Y-%m-%dT%H-%M-%SZ)
BACKUP_NAME="obsidian-backup-${TIMESTAMP}.tar.gz.gpg"
TMPDIR=$(mktemp -d)

log() { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [obsidian] $*" | tee -a "$LOG_FILE"; }
...
if [[ ! -d "/root/obsidian-vault" ]]; then
  log "ERROR: /root/obsidian-vault not found — skipping"
  exit 1
fi
...
tar -czf "$ARCHIVE" \
  --ignore-failed-read \
  --exclude="/root/obsidian-vault/.git" \
  /root/obsidian-vault/ \
  2>/dev/null || true
...
UPLOAD_RESPONSE=$(curl -s -X PUT \
  "https://graph.microsoft.com/v1.0/me/drive/root:/Orion-Shared/obsidian-backups/${BACKUP_NAME}:/content" \
...
LIST_RESPONSE=$(curl -s \
  "https://graph.microsoft.com/v1.0/me/drive/root:/Orion-Shared/obsidian-backups:/children?\$select=id,name,lastModifiedDateTime&\$orderby=lastModifiedDateTime+desc" \
  -H "Authorization: Bearer $ACCESS_TOKEN")
